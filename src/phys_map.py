...
def plot_flux_map(clists, pid_class, dir_phys, title, name_out, bins, ranges, roi, do_save_json=False):
    ...
    hist2d_res = plot_longlatvar(x_edges_list, y_edges_list, flux_list, title=title, 
                    ...
                    cmap="RdYlBu_r", font_color="black")    
    plt.tight_layout()        
...
def create_phys_map(
    ...
    ,bins : list =              [180,90]    
    ,ranges : list =            [[-180,180],[-90,90]]     
    ):


    os.makedirs(dir_phys, exist_ok=True)    
    dir_phys_map = os.path.join(dir_phys, "map")
    os.makedirs(dir_phys_map, exist_ok=True)    
    ...
    # plot
    plot_flux_map(clists, [0,1,2],  dir_phys_map, "flux of all particles", "map_flux_all",bins, ranges,roi, do_save_json=True)
    plot_flux_map(clists, [0,2],    dir_phys_map, "flux of protons and ions", "map_flux_protons_ions",bins, ranges,roi, do_save_json=True)
    plot_flux_map(clists, [1],      dir_phys_map, "flux of electrons and photons", "map_flux_electrons_photons",bins, ranges,roi, do_save_json=True)
    plot_flux_map(clists, [2],      dir_phys_map, "flux of ions", "map_flux_ions",bins, ranges,roi, do_save_json=True)
    plot_flux_map(clists, [0],      dir_phys_map, "flux of protons", "map_flux_protons",bins, ranges,roi, do_save_json=True)
    ...
    for idx, var_prot in enumerate(vars_prot):
        ...
        file_fig_path_name = os.path.join(dir_phys_map, name_out + ".png")
        file_json_path_name = os.path.join(dir_phys_map, name_out + ".json")
        ...
        plot_longlatvar(long_prot, lat_prot, var_prot, title="map of protons " + vars_names[idx], 
                        ...
                        do_save_json=True, file_json_path_name=file_json_path_name, font_color="black")
        plt.tight_layout()        
        plt.savefig(file_fig_path_name, transparent=False, dpi=600)
        plt.close()
    ...
    plot_clusters(clusters_ion_saa, 15, fig=fig,ax=axes[2], title="visulisation of ions", 
                    z_label="E [keV]", do_show=False)    
    plt.tight_layout()
    plt.savefig(os.path.join(dir_phys, "cluster_visual.png"), transparent=False, dpi=600)
    plt.close()    
